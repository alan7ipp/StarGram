...
import hashlib, time, requests, re
...
TURSO_URL = "https://stargram-alan7ipp.aws-ap-northeast-1.turso.io"
...
def turso_execute(sql, params=()): turso_query_all(sql, params)
def turso_fetchone(sql, params=()): rows = turso_query_all(sql, params); return rows[0] if rows else None

def init_db():
    turso_execute("CREATE TABLE IF NOT EXISTS users (tg_id INTEGER PRIMARY KEY, username TEXT, balance REAL DEFAULT 0, total_spent REAL DEFAULT 0, banned INTEGER DEFAULT 0, ban_reason TEXT DEFAULT '', ban_until REAL DEFAULT 0, created_at REAL DEFAULT 0)")
...
def send_telegram(chat_id, text):
    try: requests.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id": chat_id, "text": text, "parse_mode": "HTML"}, timeout=5)
    except: pass

@app.route("/")
def home(): return "StarGram API OK"
...
def deposit_confirm():
    data = request.json; tg_id = data.get("tg_id"); amount = float(data.get("amount", 0)); tx_hash = data.get("tx_hash", "")
    ...
    user = turso_fetchone("SELECT balance FROM users WHERE tg_id = ?", (tg_id,)); new_balance = user["balance"] if user else amount
    send_telegram(tg_id, f"{E['coin']} <b>Баланс пополнен!</b>\n\n{E['money']} Сумма: <b>+{amount} GRAM</b>\n{E['coin']} Новый баланс: <b>{new_balance} GRAM</b>\n\n{E['check']} Средства зачислены!")
    for aid in ADMIN_IDS: send_telegram(aid, f"{E['money']} <b>Пополнение!</b>\n\n{E['user']} ID: <b>{tg_id}</b>\n{E['money']} +{amount} GRAM\n{E['coin']} Баланс: <b>{new_balance} GRAM</b>")
    return jsonify({"status": "ok", "balance": new_balance})

@app.route("/api/order", methods=["POST"])
def order():
    data = request.json; tg_id = data.get("tg_id"); otype = data.get("type"); price = float(data.get("price", 0))
    qty = data.get("quantity"); rec = data.get("recipient"); name = data.get("name"); pfb = data.get("paid_from_balance", False)
    ...
    if pfb:
        ...
        if user and user["balance"] >= price:
            ...
            user_updated = turso_fetchone("SELECT balance FROM users WHERE tg_id = ?", (tg_id,)); new_balance = user_updated["balance"] if user_updated else 0
            if otype == 'stars': item_icon = E['star']; item_text = f"{qty} звёзд"
            else: item_icon = E['crown']; item_text = f"Premium на {qty} мес."
            send_telegram(tg_id, f"{E['check']} <b>Покупка совершена!</b>\n\n{E['id_icon']} Заказ <b>#{oid}</b>\n{item_icon} Товар: <b>{item_text}</b>\n{E['msg']} Получатель: <b>{rec}</b>\n{E['money']} Списано: <b>{price} GRAM</b>\n{E['coin']} Остаток: <b>{new_balance} GRAM</b>\n\n{E['clock']} Ожидайте отправки!")
            for aid in ADMIN_IDS: send_telegram(aid, f"{E['diamond']} <b>Новая покупка!</b>\n\n{E['id_icon']} Заказ <b>#{oid}</b>\n{E['user']} ID: <b>{tg_id}</b>\n{item_icon} Товар: <b>{item_text}</b>\n{E['msg']} Получатель: <b>{rec}</b>\n{E['money']} Сумма: <b>{price} GRAM</b>\n\n{E['lightning']} <b>Зайди на Fragment и отправь!</b>")
            return jsonify({"status": "ok", "balance": new_balance})
        else: return jsonify({"error": "insufficient"}), 400
...
def user_orders(tg_id):
    ...
    return jsonify([{"id": o["id"], "item_name": o["item_name"], "recipient": o["recipient"], "amount": o["amount"], "status": o["status"]} for o in orders])

@app.route("/api/telegram/user/<username>")
def telegram_user(username):
    try:
        resp = requests.get(f"https://t.me/{username}", headers={"User-Agent": "TelegramBot"}, timeout=10)
...
def admin_action(action):
    data = request.json; admin_id = data.get("admin_id")
    ...
    if action == "userinfo":
        ...
        return jsonify({"error": "not found"}), 404
    elif action == "ban": turso_execute("UPDATE users SET banned=1, ban_reason=?, ban_until=? WHERE tg_id=?", (data.get("reason",""), time.time()+float(data.get("hours",0))*3600 if float(data.get("hours",0))>0 else 0, uid))
    elif action == "unban": turso_execute("UPDATE users SET banned=0 WHERE tg_id=?", (uid,))
    elif action == "give": turso_execute("UPDATE users SET balance = balance + ? WHERE tg_id = ?", (float(data.get("amount",0)), uid))
    elif action == "take": turso_execute("UPDATE users SET balance = balance - ? WHERE tg_id = ?", (float(data.get("amount",0)), uid))
